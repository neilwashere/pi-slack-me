...
Retention lives at `~/.pi/agent/pi-slack-me-inbox-v3-<identity-hash>.json` (override with `PI_SLACK_INBOX_STORE`), holds at most 1,000 unacked messages, and is written `0600`. For `xoxp-` and rotating `xoxe.xoxp-` tokens the hash uses stable workspace/user/app segments, so app-token and user-token rotation keep both the same inbox and sidecar lock without exposing token material. Exactly one sidecar owns the file through a writer lock derived from the store path, including custom overrides. Messages are deduplicated on channel and timestamp, and the last 1,000 completed keys are remembered, so recently acked messages are not admitted twice. An unreadable or old-format store is preserved with a unique `.corrupt-<timestamp>-<pid>-<id>` suffix, recovery starts empty, and a unique `0600` recovery log records the location. If the file cannot be quarantined, the footer reports `durability disabled`; the sidecar then keeps work in memory without overwriting the damaged file.
...
Each scope starts no earlier than 24 hours before the pass. Conversation and thread scopes read at most five pages of 100 messages; mention search reads up to 20 pages to absorb Slack's day-granularity search overlap. The whole pass scans at most 200 relevant messages. Live and recovered messages share the `channel_id:timestamp` dedupe key. When newest-first conversation history reaches a bound, a durable backward boundary lets the next call continue into older pages instead of repeating the same page window. The result reports `truncated` and per-scope `errors`; either means the delta was not proven complete. Without `search:read`, known conversations and threads still reconcile, but mentions in previously unknown channels cannot be recovered. Discovering DMs requires `im:read` and `mpim:read`; missing scopes appear in `errors` rather than suppressing other recovery scopes.
